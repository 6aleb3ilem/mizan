...
import { ImportModalComponent } from '../type/import-modal/import-modal.component';
@Component({
  selector: 'app-type',
...
export class TypeComponent implements OnInit{
  typeIdToUpdate: number | undefined;
 
  types: Type[] = [];
  updatedtype: Type | undefined;
   ...
   constructor(private typeService: TypeService, private router: Router, public dialog: MatDialog) {}
 
   ngOnInit(): void {
     this.typeService.getAllTypes().subscribe(types => {
      this.types = types;
       this.dataSource.data = types;
       this.paginatedData = this.types.slice(0, 5); // Initial pagination setup
     });
   }
   onPageChange(event: PageEvent): void {
    const startIndex = event.pageIndex * event.pageSize;
    const endIndex = startIndex + event.pageSize;
    this.paginatedData = this.types.slice(startIndex, endIndex);
  }
   arrayBuffer: any;
   file!: File;
...
sortData(sortBy: string) {
  ...
  if (sortBy === 'id') {
    this.types.sort((a, b) => {
      if (a.id_type < b.id_type) return this.sortDirection === 'asc' ? -1 : 1;
      if (a.id_type > b.id_type) return this.sortDirection === 'asc' ? 1 : -1;
      return 0;
    });
...
paginateData() {
 ...
 exportData(): void {
   ...
   XLSX.writeFile(wb, 'type_data.xlsx');
 }
   ngAfterViewInit() {
  ...
  this.paginatedData = this.types.filter(type => {
    return type.id_type.toString().includes(filterValue) || type.label.toLowerCase().includes(filterValue);
  });
  ...
  openModalDelete(typeId: number) {
    this.typeService.selectedTypeId = typeId;
